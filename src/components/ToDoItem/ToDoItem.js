...
function ToDoItem(props) {
  return (
    <li className="todo-item" key={props.task.id}>
      <div className="todo-item__title">
        {props.task.title}
        <span>{props.task.date}</span>
      </div>
      <div className="todo-item__duration">{props.task.duration}</div>
      <label className='task-done' htmlFor='task-done'>
        <input type="checkbox" name="task-done" />
        <span></span>
      </label>
      <button className='todo-item__remove' type="button" onClick={props.onRemoveTask}>
